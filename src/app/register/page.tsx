            ...
            <div className="flex gap-2">
              {oauthProviders.map((p: any) => (
                <Button key={p.id} variant="outline" className="flex-1 gap-2" onClick={() => signIn(p.id, { callbackUrl: "/" })}>
                  {p.icon ? (p.icon.startsWith("<svg") ? <span className="h-4 w-4 [&>svg]:w-full [&>svg]:h-full" dangerouslySetInnerHTML={{ __html: p.icon }} /> : <img src={p.icon} alt="" className="h-4 w-4 rounded" />) : (OAUTH_ICONS[p.id] || null)}
                  {p.name || p.id}
                </Button>
              ))}
            </div>
